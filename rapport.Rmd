...
* **18,5 $\leq$ IMC $\leq$ 24,9** : zone considérée comme normale ;
* **IMC > 25** : surpoids.
...
**En conclusion :** L'analyse des grilles montre qu'en 20 ans, le paysage olympique s'est rationalisé. Les nations qui n'ont pas la capacité des USA ou de la Chine pour briller partout ont developpé des stratégies de niche très ciblées (comme la GBR et les NED en cyclisme) pour optimiser leur présence au tableau des médailles.
...
Regardons quels sont les sports qui sont des exemples en termes t'équité.
...
ggplot(data_universel, aes(x = Ratio_Universalite, y = reorder(Sport, Ratio_Universalite))) +
  geom_segment(aes(x = 0, xend = Ratio_Universalite, y = Sport, yend = Sport), color = "lightgray", linewidth = 1) +
  geom_point(color = "#0072B2", size = 4) + # Point bleu
  labs(
    title = "Universalité des Sports (JO d'été 2000-2016)",
...
Regardons ensuite quels sont les sports qui sont les plus accaparés par certaines nations.
...
ggplot(data_monopole, aes(x = Ratio_Universalite, y = reorder(Sport, desc(Ratio_Universalite)))) +
  ...
  labs(
    title = "Les Sports sous 'Monopole' (JO d'été 2000-2016)",
...
ggplot(logistique_treemap, aes(
  ...
  labs(
    title = "Répartition spatiale du Village Olympique (2000-2016)",
...
ggsave("treemap.png", width = 10, height = 6, dpi = 320)
...
D'autre part, on remarque qu'on arrive à une conclusion qui contredit celle du point précédent : Les sports collectifs sont certes couteux, mais ce sont aussi eux qui offrent le plus d'égalité des chances.
...
# Compter le nombre d'épreuves par an
...
ggplot(donnees_parite, aes(x = Year, y = Valeur_Graphique, fill = Type_Epreuve)) +
  ...
  labs(
    title = "La route vers la parité du programme",
...
On part du principe que si un athlète a participé à l'épreuve, c'est que cette dernière était au programme. Regardons à l'aide d'un diagramme de Gantt la pérennité des différents sports.

```{r}
# Création du graphique (Frise / Gantt)
ggplot(timeline_data, aes(x = Year, y = reorder(Sport, desc(Sport)))) +
  ...
  labs(
    title = "Le 'Cimetière' et l'Héritage Olympique",
...
Le graph manque de clarté : les lignes discontinues atténuent la lisibilité du diagramme de Gant. De plus, tous les sports sont ici condensés : comme il s'agit d'une liste, augmentons la hauteur du graph.

```{r fig.height=12, fig.width=8}
ggplot(timeline_data, aes(x = Year, y = reorder(Sport, desc(Sport)))) +
  
  # LA SOLUTION EST ICI : geom_tile remplace geom_point
  # width = 4 relie parfaitement les éditions. height = 0.6 contrôle l'épaisseur de la barre
  geom_tile(aes(width = 4, height = 0.6), fill = "#0072B2") +
  ...
  theme(
    plot.title = element_text(face = "bold", size = 14),
    # J'ai légèrement remonté la taille du texte à 8.5 pour que ce soit plus lisible
...
Cela pourrait être également intéressant de confronter l'apparition des épreuves ainsi que leur nombre de médailles : un sport qui distribue 2 médailles est plus enclin à disparaitre qu'un sport qui en distribue 20.

```{r fig.height=12, fig.width=8}
data_soundwave <- athletes_full %>%
  filter(Season == "Summer") %>%
  group_by(Sport, Year) %>%
  summarise(Nb_Epreuves = n_distinct(Event), .groups = 'drop') %>%
  
  group_by(Sport) %>%
  mutate(Annee_Apparition = min(Year)) %>%
  ungroup() %>%
  
  # LA CORRECTION EST ICI : 
  # On donne 0.15 d'épaisseur de base obligatoire, plus un bonus proportionnel (max 0.75)
  mutate(Hauteur_Norm = 0.1 + (Nb_Epreuves / max(Nb_Epreuves)) * 0.75)

# CRÉATION DU GRAPHIQUE

ggplot(data_soundwave, aes(x = Year, y = reorder(Sport, desc(Annee_Apparition)))) +
  
  # La Tuile : La largeur est fixe (4 ans), mais la HAUTEUR varie selon nos données !
  geom_tile(aes(width = 4, height = Hauteur_Norm), fill = "#0072B2") +
  
  # Repères historiques
  geom_vline(xintercept = c(1916, 1940, 1944), linetype = "dashed", color = "red", alpha = 0.5) +
  
  labs(
    title = "L'Inflation du Programme Olympique",
    subtitle = "Tri par date d'apparition. L'épaisseur indique le nombre d'épreuves (Podiums).",
    x = "Année", y = ""
  ) +
  theme_minimal() +
  theme(
    ...
    panel.grid.minor = element_blank(),
    panel.grid.major.y = element_line(color = "lightgray", linetype = "dotted") # Aide à lire les lignes fines
  )

```

Le constat est flagrant : non seulement le CIO empile de nouveaux sports vers le bas du graphique (la modernité), mais surtout, les "Piliers" situés en haut du graphique s'épaississent avec le temps ! Regardez la ligne de la Natation ou de la Gymnastique : elles gonflent comme des ondes sonores au fil des décennies. Le problème du CIO aujourd'hui n'est pas seulement le nombre de sports, c'est l'inflation des épreuves à l'intérieur même des disciplines historiques.

Pour illustrer la dynamique concurrentielle entre les disciplines, nous avons fait le choix de compléter notre approche analytique par un outil interactif de visualisation : Power BI. Là où le graphique précédent cartographie l'empreinte historique des sports de manière statique via l'épaisseur de ses traits, le graphique en barres animé présent dans notre tableau de bord restitue la véritable "course au gigantisme" ; nous vous invitons donc vivement à ouvrir le fichier joint pour expérimenter cette animation et observer en temps réel quels sports en ont dépassé ou cannibalisé d'autres décennie par décennie.

***Pour conclure cette partie*** 

En conclusion, notre analyse montre que le CIO doit trouver le bon équilibre entre son héritage historique et les attentes d'aujourd'hui. Face aux limites logistiques et financières des Jeux, il est devenu indispensable de rationaliser l'événement et d'optimiser la place de chaque discipline.

Cependant, faire des économies ne suffit pas : il faut aussi savoir se réinventer. L'intégration de nouveaux sports ciblés et l'engagement fort vers la parité sont essentiels pour continuer à attirer le public. Dans ce contexte, la data n'est plus seulement là pour dresser des bilans : c'est un véritable outil de pilotage qui permet au CIO de prendre les bonnes décisions pour l'avenir des Jeux Olympiques.
...
Bien sûr, ce travail d'analyse s'est parfois accompagné de quelques défis techniques, notamment lorsqu'il a fallu nettoyer, restructurer la donnée ou contourner les limites de certains outils pour obtenir les visualisations interactives souhaitées.

Ces étapes nous ont cependant donné de nombreuses idées pour aller plus loin et élargir la portée de notre étude. Dans un premier temps, il serait très pertinent d'actualiser notre jeu de données avec les résultats des éditions les plus récentes (notamment Tokyo 2020 et Paris 2024). Cela permettrait de confronter nos analyses à l'actualité et de vérifier si les objectifs fixés par le CIO (comme le plafond strict du nombre d'athlètes ou l'atteinte totale de la parité) ont bien été franchis.

Enfin, pour donner une dimension encore plus stratégique à ce projet, une excellente piste serait de croiser nos informations avec d'autres jeux de données. Intégrer des indicateurs socio-économiques (comme le PIB, la démographie ou les investissements publics de chaque nation) ou ajouter les résultats d'autres compétitions internationales majeures permettrait de comprendre encore plus finement les véritables moteurs de la performance sportive à l'échelle mondiale.
...
**Margot :** Ce projet m’a permis de me mettre dans une logique plus orientée utilisateur en cherchant à construire des visualisations qui facilitent réellement l’exploration et la comparaison des sports. Le développement de l’onglet d’analyse par sport sur Shiny m’a amenée à faire évoluer plusieurs graphiques pour les rendre plus interactifs et plus lisibles. Avec plus de temps, il aurait été intéressant d’ajouter davantage d’analyses selon le genre afin d’étudier plus finement les différences morphologiques entre athlètes.

**Lubin :** Ce projet a été très intéressant dans la prise de décision sur quelle visualisation pouvait correspondre le mieux afin d'afficher l'information voulue, mais aussi dans le choix de sa pertinence : est-ce que cette visualisation est utile ? Est-ce qu'elle demande d'autres visuels complémentaires, comme une comparaison avec une autre année ? Pour aller plus loin, il aurait été intéressant d'ajouter une dimension géographique (cartographie interactive par continent) pour analyser visuellement si l'efficacité sportive se regroupe par régions du monde.

**Florian :** Ce projet a été une excellente opportunité pour découvrir la datavisualisation tout en approfondissant mes connaissances sur les Jeux Olympiques.  Il aurait été intéressant, avec d'avantage de temps, de croiser notre dataset avec d'avantages d'informations, tel que les budgets exacts de chaque édition pour pousser encore plus loin notre analyse financière.
