...
```{r}
# 1. Calcul des proportions par genre et par édition
...
# 2. Identification de la première année d'apparition de CHAQUE sport
df_debut_sports <- athletes_full %>%
  group_by(Sport) %>%
  summarise(Annee_Debut = min(Year, na.rm = TRUE), .groups = "drop")

# 3. Calcul de l'année de parité et jointure avec l'année de début
...
ggplot(df_parite_sports, aes(x = reorder(Sport, -Annee_Parite), y = Annee_Parite)) +
  ...
  # Point matérialisant l'année où la parité est atteinte
  geom_point(color = "#1D9E75", size = 3.5) +
  
  # Point optionnel pour matérialiser le début historique (très élégant visuellement)
  geom_point(aes(y = Annee_Debut), color = "grey60", size = 1.5, shape = 1) +
  
  # Ajustement du texte pour éviter qu'il ne chevauche les points
